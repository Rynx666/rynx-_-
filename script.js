...
let lastSyncedBalance = 0; // сколько последний раз синкали на сервер
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    // --- Кнопка "Друзья" ---
    document.querySelector('.veln-nav-btn[data-tab="friends"]').onclick = () => {
        const tab = document.getElementById("tab-friends");
        const btn = document.querySelector('.veln-nav-btn[data-tab="friends"]');
        if (tab.classList.contains("hidden")) {
            tab.classList.remove("hidden");
            btn.classList.add("active");
            document.getElementById("tab-inventory").classList.add("hidden");
    ...
    // --- Кнопка "Рынок" ---
    document.querySelector('.veln-nav-btn[data-tab="market"]').onclick = () => {
        const tab = document.getElementById("tab-market");
        const btn = document.querySelector('.veln-nav-btn[data-tab="market"]');
        if (tab.classList.contains("hidden")) {
            tab.classList.remove("hidden");
            btn.classList.add("active");
            document.getElementById("tab-inventory").classList.add("hidden");
            ...
            document.querySelectorAll('.veln-nav-btn').forEach(b => {
    ...
    // --- Кнопка закрытия истории подарков (новая отдельная кнопка!) ---
    ...
    // --- MARKET: открыть модалку продажи ---
...
async function loadUserData() {
    try {
        ...
        if (balance > serverBalance) {
            // Локальный баланс выше — отправим на сервер
            await saveToServer();
            lastSyncedBalance = balance;
        } else {
            // Серверный больше или равен — используем его
...
// Раз в 15 секунд синхронизируем баланс (если он изменился)
...
function stopCoinTimerAndSave() {
    ...
    saveToServer();
    saveToLocalCache();
    // Дополнительно синхронизируем баланс при уходе
...
async function openCase() {
    ...
    try {
        ...
        if (result.success) {
            ...
            lastSyncedBalance = balance; // синхронизирован
...
// --- MARKET: получить и отобразить рынок ---
...
// --- ДРУЗЬЯ: заявки, список, принятие ---

// Вкладка "Друзья": заявки и друзья
async function loadFriendsTab() {
    await loadFriendRequests();
    await loadFriendsList();
}

// Получить заявки в друзья (входящие + исходящие)
...
// Кнопка "Принять" заявку в друзья
...
// --- Друзья: список (оставлено как было, но вызывается только из loadFriendsTab) ---
...
// --- Автообновление баланса и инвентаря (polling) ---
